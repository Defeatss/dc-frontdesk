            ...
            options = {
                {
                    ...
                    action = function()
                        ...
                        TriggerEvent("dc-frontdesk:client:OpenFrontDesk", location)
                    end,
                    icon = "fas fa-desktop",
                    label = "Front Desk",
                },
...
-- Front Desk Menu
RegisterNetEvent('dc-frontdesk:client:OpenFrontDesk',function(job)
    if PlayerJob.name == job then
        local FrontDeskMenu = {
            ...
            },
            {
                ...
                params = {
                    event = 'dc-frontdesk:client:ToggleDuty',
                    args = job
                }
            },
            {
                ...
                params = {
                    event = 'dc-frontdesk:client:OpenAssistanceMenu',
                    args = job
                }
            },
            {
...
-- Toggle Duty Event
RegisterNetEvent('dc-frontdesk:client:ToggleDuty',function(job)
    if job == 'police' then
...
-- Assistance Menu
RegisterNetEvent('dc-frontdesk:client:OpenAssistanceMenu',function(job)
    ...
    AssistanceMenu[#AssistanceMenu + 1] = {
        ...
        params = {
            event = 'dc-frontdesk:client:OpenFrontDesk'
        }
    }
...
RegisterNetEvent('dc-frontdesk:client:RequestAssistance',function(data)
    QBCore.Functions.TriggerCallback('dc-frontdesk:server:CooldownCheck', function(alert)
        if alert then
            if Config.Dispatch == "default" then
                if data.job == 'police' then
                    if CurrentCops >= Config.Locations[data.job].Required then
                        ...
                        TriggerServerEvent('dc-frontdesk:server:AlertCooldown', data.job, true)
                    else
